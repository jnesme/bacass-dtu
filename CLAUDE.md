...
- **Funcscan `GECCO_RUN` — "mv: are the same file"**: GECCO outputs flat files named `${input.baseName}.*`. When `input.baseName` == `meta.id`, the mv is a rename-to-self. Fix: `ext.prefix = { "${meta.id}_gecco" }` in `conf/funcscan_overrides.config` — mv renames `S0204.*` → `S0204_gecco.*` and succeeds. NF collects outputs normally.
- **Funcscan `DEEPBGC_PIPELINE` — mv double-suffix bug**: DeepBGC creates a subdirectory named `${prefix}/`. After renaming the directory (`genome.baseName/` → `prefix/`), the file-level mv loop does `${i/genome.baseName/prefix}` on full paths. Since `prefix` (`S0344_deepbgc`) contains `meta.id` (`S0344`), the substitution also hits the directory name in the path → `S0344_deepbgc_deepbgc/` which doesn't exist → error. `ext.prefix` cannot fix this (any prefix containing `meta.id` reproduces the bug). Fix: `errorStrategy = 'ignore'` for `DEEPBGC_PIPELINE`. The pipeline completes successfully; COMBGC runs on antiSMASH + GECCO results only.
