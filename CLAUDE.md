...
**FastQC CPU/maxForks rationale**: FastQC supports multi-threading via `-t`: each thread processes one file in parallel. `cpus=2` + `-t 2` processes R1 and R2 simultaneously for paired-end data — honest and efficient. `maxForks=10` + `order[-slots]` caps total concurrent instances and spreads them across nodes. The original incident (1.8M CS/s, Mar 2026) was caused by no `maxForks`: LSF packed 10 JVMs onto one node. With `maxForks=10` and `order[-slots]`, at most 1–2 jobs land per node. **Note**: with the LSF executor, `maxForks` defaults to effectively unlimited (the head job has 1 CPU; `maxForks` default = CPUs−1 applies to local executor only). Must be set explicitly for any tool where actual resource use differs from nf-core defaults.
