...
- **Funcscan `GECCO_RUN` / `DEEPBGC_PIPELINE` — "mv: are the same file" (exit 1, ignored — results unaffected)**: After the tool runs, the module does `mv ${input.baseName}.* ${prefix}.*` to standardise output names. Since `input.baseName` (`S0204`) equals `prefix` (`meta.id` = `S0204`), every `mv` is a rename-to-self and fails with exit 1. The tool output is already correctly named — all declared output files (`*.genes.tsv`, `*.features.tsv`, `*.clusters.tsv`, `*_cluster_*.gbk`) are present in the work directory before the mv runs. With `errorStrategy = 'ignore'` (GECCO) and `errorStrategy = { task.exitStatus == 1 ? 'ignore' : 'retry' }` (DeepBGC) in `conf/funcscan_overrides.config`, Nextflow marks the task as ignored, collects the already-present outputs, and passes them to downstream processes normally. **The BGC results are complete and correct.** To verify: check the work directory hash shown in the `NOTE:` log line — all expected output files should be present. This will trigger for every sample whose name matches the assembly basename, which is always the case when bacass names assemblies after the sample ID.
