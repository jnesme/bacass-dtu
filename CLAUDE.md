...
- **Unicycler jobs stuck PEND — "resource reservation not satisfied" or "queue mem limit reached"**: root cause is `perJobMemLimit` not set to `false` in `conf/lsf.config`. In NF 25.x the default is `true`, which **disables** CPU-based division (`shouldDivide = !perJobMemLimit`), so LSF receives the full 80 GB as per-slot — multiplying by 16 CPUs = 1280 GB total, exceeding the ~1.07 TB queue limit. Fix: ensure `perJobMemLimit = false` is explicitly set in `conf/lsf.config`. Tell-tale sign: `-M` is correctly divided (e.g. 5120) but `rusage[mem=81920]` is not. Kill the head job and resubmit with `-resume`.
