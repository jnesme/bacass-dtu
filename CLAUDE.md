...
- **Any funcscan task — `python: can't open file '/work3/josne/miniconda3/bin/conda': [Errno 2] No such file or directory`** (exit code 2): NFS intermittent failure when Python tries to open the conda entry-point script as a script file. `python script.py` always calls `open()` on the script — if NFS fails to serve it, Python reports this error. Fix: replace `/work3/josne/miniconda3/bin/conda` with a bash wrapper using `python -c '...'` (inline code, no NFS `open()` needed). Original backed up to `conda.bak`. Also compile the conda package to UNCHECKED_HASH so imports don't read `.py` from NFS:
  ```bash
  # The wrapper is already in place. To reapply if conda is updated:
  cp /work3/josne/miniconda3/bin/conda.bak /work3/josne/miniconda3/bin/conda  # restore original first
  cat > /work3/josne/miniconda3/bin/conda << 'WRAPPER'
  #!/bin/bash
  ...
  ' "$@"
  WRAPPER
  chmod +x /work3/josne/miniconda3/bin/conda
  /work3/josne/miniconda3/bin/python -c "import compileall,py_compile; compileall.compile_dir('/work3/josne/miniconda3/lib/python3.13/site-packages/conda', invalidation_mode=py_compile.PycInvalidationMode.UNCHECKED_HASH, quiet=1, workers=0)"
  ```
  Verify: `conda --version` and `conda shell.bash hook` should both work.
