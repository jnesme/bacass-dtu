...
- **Funcscan `GECCO_RUN` / `DEEPBGC_PIPELINE` — "mv: are the same file" → results missing from COMBGC**: After the tool runs, the module renames outputs from `${input.baseName}.*` to `${prefix}.*`. Since `input.baseName` (`S0204`) equals `prefix` (`meta.id` = `S0204`), the mv is a rename-to-self and fails with exit 1. **`errorStrategy = 'ignore'` does NOT work** — it drops the output channel entirely, so GECCO/DeepBGC results never get published or flow to COMBGC. Fix (in `conf/funcscan_overrides.config`): set `ext.prefix = { "${meta.id}_gecco" }` (and `_deepbgc` for DeepBGC) so mv renames `S0204.*` → `S0204_gecco.*` — a different name — and succeeds. NF then collects outputs normally. The suffix is cosmetic; downstream COMBGC uses `meta.id` for sample identity, not the filename.
