process UNICYCLER {
    ...
    # so any rm there targets the NFS work dir, not the scratch dir where spades_assembly/ lives.
    if [ -d spades_assembly ]; then
        echo "WARNING: removing stale SPAdes checkpoint from a previous job"
        rm -rf spades_assembly/ 2>/dev/null || true
    else
