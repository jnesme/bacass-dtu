...
flowchart TD
    ...
    subgraph sqc ["Short read QC & trimming"]
        FASTP["FASTP\ntrim + QC"]
        FASTQC["FastQC"]
        FASTP --> FASTQC
    end

    subgraph lqc ["Long read QC & filtering"]
        NP["NanoPlot / ToulligQC"]
        PC["PycoQC\n(Fast5 only)"]
        FI["Porechop or Filtlong"]
    end

    subgraph cont ["Contamination & species ID"]
        K2["Kraken2"]
        KMF["KmerFinder"]
    end

    subgraph asm ["Assembly — choose one"]
        direction LR
        UNI["Unicycler\nshort · hybrid"]
        CAN["Canu\nlong"]
        MIA["Miniasm + Racon\nlong"]
    ...
    subgraph pol ["Polishing — long / hybrid only"]
        direction LR
        MED["Medaka"]
        NAN["Nanopolish\n(requires Fast5)"]
    end

    subgraph aqc ["Assembly QC"]
        QST["QUAST"]
        BSC["BUSCO"]
    end

    subgraph ann ["Annotation — choose one"]
        direction LR
        BAK["Bakta ★\n(default)"]
        PRO["Prokka"]
        DFS["DFAST"]
...
> **Fair usage**: `queueSize = 50` allows up to ~400 cores running simultaneously. DTU HPC uses fairshare scheduling — sustained high-utilization runs deplete your priority and can cause subsequent jobs to PEND for hours or days. If jobs are stuck PEND for >24h, fairshare may be exhausted: kill the run, wait for priority to recover (typically overnight), then resubmit with `-resume` to pick up where you left off.
...
| miniasm | 8 | 40 GB | Multi-threaded and RAM-intensive; 8 CPUs / 40 GB matches RACON/MEDAKA on the same long-read path |
